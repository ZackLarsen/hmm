...
WSJ_train = os.path.join(homedir, 'data\\WSJ_train.txt')
WSJ_test = os.path.join(homedir, 'data\\WSJ_test.txt')
...
token_list, pos_list, token_pos_list = file_prep(WSJ_head,
                                                 nrows=100,
                                                 lowercase=True)
token_list
pos_list
...
reverse_token_map = reverse_token_map(token_map)
reverse_token_map
type(reverse_token_map)
...
## (dimensions (n_unique_tags * n_unique_tags))
#matrix_a = np.zeros((n_unique_tags, n_unique_tags))
## OR (n_unique_tags + 1) * (n_unique_tags + 1) to include START and END:
matrix_a = np.zeros(((n_unique_tags + 1), (n_unique_tags + 1)))
